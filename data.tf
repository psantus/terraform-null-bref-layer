...
locals {
  ...
  bref_catalog_url       = var.bref_catalog_url != null ? var.bref_catalog_url : "https://raw.githubusercontent.com/brefphp/bref/master/layers.json"
  ...
  function_layer_key = "${local.bref_layer_name_prefix}${local.cpu_prefix}php-${local.php_version_normalized}"
  fpm_layer_key      = "${local.bref_layer_name_prefix}${local.cpu_prefix}php-${local.php_version_normalized}-fpm"
  console_layer_key  = "${local.bref_layer_name_prefix}console"
  ...
  extension_layer_data = {
    for ext in var.php_extensions : ext => {
      ...
      version   = lookup(local.extension_versions["${ext}-php-${local.php_version_normalized}"], var.aws_region, null)
    }
  }
